...
def addMultiSplats(originalImage, mudSplatObjects):
    ...
    for splatObj in mudSplatObjects:
        ...
        # Calculate standard deviation for Gaussian Blur using Original Image - to
        # be applied to the mud-splat for overlaying
        ret = cv2.threshold(np.uint8(originalImage[int(0.25*imgW):int(0.75*imgW),int(0.25*imgH):int(0.75*imgH),1]),0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[0]
        temp_wh_half = originalImage[int(0.25*imgW):int(0.75*imgW),int(0.25*imgH):int(0.75*imgH),1][originalImage[int(0.25*imgW):int(0.75*imgW),int(0.25*imgH):int(0.75*imgH),1]>=ret]
        sigma = np.std(temp_wh_half)
        # Apply Gaussian Blur to Mud Splat
        testSplat = cv2.GaussianBlur(testSplat, (5, 5), sigma)
    ...
    allMudSplats = 260+np.zeros((imgW, imgH, 4))
    allMudSplats[:,:,3] = np.zeros((imgW, imgH))
    for i, splat in enumerate(finalSplats):
        allMudSplats[yOffset[i]:yOffset[i]+splat.shape[0],
                    xOffset[i]:xOffset[i]+splat.shape[1], :3] = np.minimum(splat[:,:,:3],
                        allMudSplats[yOffset[i]:yOffset[i]+splat.shape[0],
                                    xOffset[i]:xOffset[i]+splat.shape[1], :3])
        allMudSplats[yOffset[i]:yOffset[i]+splat.shape[0],
                    xOffset[i]:xOffset[i]+splat.shape[1], 3] = np.maximum(splat[:,:,3],
                        allMudSplats[yOffset[i]:yOffset[i]+splat.shape[0],
                                    xOffset[i]:xOffset[i]+splat.shape[1], 3])
    allMudSplats[allMudSplats==260] = 0
    allMudSplats = allMudSplats.astype("uint8")
    alpha_s = allMudSplats[:, :, 3].astype("float32")/255
    alpha_l = 1.0 - alpha_s
    muddyImage = copy.deepcopy(originalImage)
