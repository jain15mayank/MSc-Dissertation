...
folder = '../../GTSRB Dataset/categorized&cropped_NA - Test/'
...
y_turnLeft = np.zeros((turnLeft1.shape[0], 3))
y_turnLeft[:,0] = 1
score = model.evaluate(turnLeft1, y_turnLeft, verbose=0)
print("Turn Left %s: %.2f%%" % (model.metrics_names[1], score[1]*100))

y_turnRight = np.zeros((turnRight1.shape[0], 3))
y_turnRight[:,1] = 1
score = model.evaluate(turnRight1, y_turnRight, verbose=0)
print("Turn Right %s: %.2f%%" % (model.metrics_names[1], score[1]*100))

y_goStraight = np.zeros((goStraight1.shape[0], 3))
y_goStraight[:,2] = 1
score = model.evaluate(goStraight1, y_goStraight, verbose=0)
print("Go Straight %s: %.2f%%" % (model.metrics_names[1], score[1]*100))